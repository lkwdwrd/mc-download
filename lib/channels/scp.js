...
function privateKey(config){
	if(!config.opts.privateKey){
		_.assign(config.opts, getKey(config.opts.host));
	}
	return config;
}
...
function promptPassphrase(config, resolve, reject){
	config.request.adapter.prompt([{
		type: 'password',
		name: 'passphrase',
		message: 'Passphrase for '+config.opts.keyName+':'
	}], function processAnswers( answers ){
		_.assign(config.opts, getKey.cachePassphrase(config.opts.host, answers.passphrase));
		resolve(config);
	});
}
...
function getStream(file){
	file.stream = file.config.request.client.__sftp.createReadStream(file.location).pause();
